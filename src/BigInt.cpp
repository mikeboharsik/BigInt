...
BigInt BigInt::operator%(const BigInt& right)
{
  if (BigInt(data) < BigInt(right.data))
    return BigInt(data);
  else if (BigInt(data) == BigInt(right.data))
    return BigInt("0");
  return remainderOfDivision(data, right.data);
}
...
BigInt BigInt::remainderOfDivision(const string& numerator, const string& denominator)
{
  if (BigInt(numerator) < BigInt(denominator))
    return 0;
  else
  {
    ...
    for (BigInt i = BigInt(0);; i = BigInt(i) + BigInt(1))
    {
      l = l - r;
